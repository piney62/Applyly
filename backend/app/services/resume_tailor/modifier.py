...
# Styles that must never be modified â€” Word Heading-style paragraphs
PROTECTED_STYLES: set[str] = {"heading 1", "heading 2", "heading 3", "heading 4",
                              "title", "subtitle", "heading1", "heading2", "heading3"}

# Contact info pattern â€” paragraphs with email/URL/phone are never edited
...
def rewrite_para(para: Any, new_text: str) -> None:
    ...
    Smart run-split for Skills paragraphs:
      ...
      Naive approach: puts everything in runs[0] â†’ entire line becomes bold (WRONG)
    ...
    if len(runs) >= 2:
        ...
        if is_label:
            sep = new_text.find(': ')
            if sep != -1:
                runs[0].text = new_text[:sep + 2]   # label part â€” keeps bold
                runs[1].text = new_text[sep + 2:]   # content â€” keeps normal weight
                for run in runs[2:]:
                    run.text = ""
                return
...
def skills_label_guard(old_text: str, new_text: str) -> str:
    ...
    if not label_match:
        return new_text  # no label in this paragraph â€” use as-is
    ...
    if new_label_match:
        ...
        # Label was changed â€” restore original label
        rest = new_text[new_label_match.end():]
        return original_label + rest
    else:
        # Label was removed entirely â€” prepend original label
...
def _split_skill_tokens(body: str) -> list[str]:
    ...
    â†’ ["Node.js (Express, NestJS)", "Python (Flask/Django/FastAPI)", "Spring Boot (Java)"]
    """
    tokens: list[str] = []
    buf: list[str] = []
    depth = 0
...
def _token_core(tok: str) -> str:
    ...
    "AWS (Lambda/ECS/RDS/S3)" â†’ "aws"
    "Spring Boot (Java)"      â†’ "spring boot"
    """
...
def skills_preserve_tokens(old_text: str, new_text: str) -> str:
    ...
    Rules (applied in order):
      1. AI's token order is preserved â€” it reflects JD-priority reordering.
      ...
      4. If the AI stripped parenthetical detail
         (e.g. "AWS (Lambda/ECS/RDS/S3)" â†’ "AWS"), the detail is restored.
    ...
    """
    # â”€â”€ Split label / body for both inputs (label_guard runs first, so the
    #    label is usually already correct; we still tolerate either shape) â”€â”€
    label_match = re.match(r'^([^:]{2,40}):\s*', old_text)
    if not label_match:
        return new_text   # not a labeled skills line â€” leave it alone
    ...
    if new_label_match and new_label_match.group(1).strip().lower() == label_key:
        ...
        new_body = new_text  # no matching label â€” treat whole thing as body
    ...
    for nt in new_tokens:
        ...
        if not nt_core or nt_core in seen_cores:
            continue   # empty or duplicate â†’ drop
...
def apply_all(sections: Sections, modifications: list[Modification]) -> ApplyStats:
    ...
    â˜… Whitelist approach (fundamental safety guarantee) â˜…
    ...
    Blocked sections: header (name, contact, portfolio, etc. â€” all paragraphs
                      before the first section heading)

    â†’ Header paragraphs are never touched no matter how the resume is formatted.
    â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    Additional protections:
      â€¢ Word Heading-style paragraphs (by Word style name)
      â€¢ Skills category label auto-restoration (original label recovered if AI deletes it)
    """
    ...
    for mod in modifications:
        ...
        item = id_map.get(pid)
        if not item:
            log.warning(f"  âš ï¸  {pid} â€” ID not found (skipping)")
            stats["skipped"] += 1
            continue
        ...
        # â•â• Whitelist check â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
        # Paragraphs not in modifiable_ids = header or section heading â†’ blocked
        if pid not in modifiable:
            log.warning(f"  ðŸš« {pid} â€” blocked (not in whitelist) [{item_section}]: "
                        f"'{old_text[:45]}'")
            stats["blocked"] += 1
            continue

        # â•â• Extra protection 1: Word Heading style â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
        style_lower = item.get("style", "").lower()
        if any(s in style_lower for s in PROTECTED_STYLES):
            log.warning(f"  ðŸ”’ {pid} â€” Heading style protected: '{old_text[:40]}'")
            stats["blocked"] += 1
            continue

        # â•â• Extra protection 2: Skills label + token preservation â•
        ...
        # â•â• No-change check â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
        cleaned_new = clean_text(new_text)
        if clean_text(old_text) == cleaned_new:
            log.info(f"  â„¹ï¸  {pid} â€” no change")
            stats["skipped"] += 1
            continue

        # â•â• Apply modification â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
        rewrite_para(item["para"], cleaned_new)
        item["text"] = cleaned_new

        log.info(f"  âœ… {pid} [{item_section}]")
        if keywords:
            log.info(f"     Keywords: {', '.join(str(k) for k in keywords)}")
        log.info(f"     Before: {old_text[:72]}{'â€¦' if len(old_text) > 72 else ''}")
        log.info(f"     After:  {cleaned_new[:72]}{'â€¦' if len(cleaned_new) > 72 else ''}")
        stats["applied"] += 1
...
def verify_key_facts(modifications: list[Modification], key_facts: list[str]) -> list[str]:
    ...
    Returns only the list of missing facts â€” never modifies text automatically.
    """
    if not key_facts:
        return []
